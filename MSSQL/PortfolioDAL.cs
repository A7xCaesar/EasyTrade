    ...
    public class PortfolioDAL : IPortfolioDAL
    {
        ...
        public async Task<List<AssetBalanceDTO>> GetAssetBalancesAsync(string userId)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                // First add cash balance
                var cashCommand = new SqlCommand(@"
                    SELECT 
                        ISNULL(SUM(CASE 
                            WHEN t.tradeType = 'sell' THEN (t.quantity * t.price)
                            WHEN t.tradeType = 'buy' THEN -(t.quantity * t.price)
                            ELSE 0
                        END), 0) as tradeBalance,
                        10000.00 as initialBalance,
                ...
                cashCommand.Parameters.AddWithValue("@UserId", userId);
                
                using (var cashReader = await cashCommand.ExecuteReaderAsync())
                {
                    if (await cashReader.ReadAsync())
                    {
                        var availableBalance = cashReader.GetDecimal("availableBalance");
                        var cashBalance = new AssetBalanceDTO
                        {
                            AssetId = "CASH",
                            Symbol = "EUR",
                            Name = "Euro Cash",
                        ...
                        assetBalances.Add(cashBalance);
                    }
                }

                // Then add crypto asset balances
                var assetCommand = new SqlCommand(@"
                    SELECT 
                        b.assetId,
                        a.symbol,
                        a.name,
                        ...
                        (b.amount * ISNULL(ph.price, 0)) as totalValue
                    FROM balances b
                    INNER JOIN assets a ON b.assetId = a.assetId
                    LEFT JOIN (
                        SELECT 
                            assetId, 
                            price,
                ...
                assetCommand.Parameters.AddWithValue("@UserId", userId);
                
                using (var assetReader = await assetCommand.ExecuteReaderAsync())
                {
                    while (await assetReader.ReadAsync())
                    {
                        var assetBalance = new AssetBalanceDTO
                        {
                            AssetId = assetReader.GetString("assetId"),
                            Symbol = assetReader.GetString("symbol"),
                            Name = assetReader.GetString("name"),
                            Amount = assetReader.GetDecimal("amount"),
                            CurrentPrice = assetReader.GetDecimal("currentPrice"),
                            TotalValue = assetReader.GetDecimal("totalValue")
                        };
        ...
        public async Task<decimal> GetTotalPortfolioValueAsync(string userId)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                var command = new SqlCommand(@"
                    ...
                    FROM AssetValues av, CashBalance cb", connection);
                command.Parameters.AddWithValue("@UserId", userId);
                ...
                return 10000.00m; // Default to initial balance if no data
            }
        }
